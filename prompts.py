...
提示词约束大模型只输出严格 JSON，字段必须与端侧 protocol.h 对齐：
  ...
  weather:   "clear"/"rain"/"snow"/"fog"  # 驱动端侧天气模式自动切换
...
4) 天气：根据画面判断当前天气 weather，取值必须为
   "clear"(晴/多云干燥) / "rain"(雨,湿滑) / "snow"(雪/冰,湿滑) / "fog"(雾/霾,湿滑视差)。
   仅依据图像证据推断（地面反光、雨滴、积雪、雾气等），不确定填 "clear"。
   设备会据此自动切换“湿滑更保守预警档”，故请如实填写。
5) feedback 综合给出：vibration(0~100 PWM)、buzzer(bool)、
